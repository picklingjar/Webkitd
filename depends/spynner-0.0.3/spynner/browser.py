...
class Browser:
    ...
    def __init__(self, qappargs=None, debug_level=None):
        ...
        self.webpage.connect(self.webpage, SIGNAL('unsupportedContent(QNetworkReply *)'), self._on_unsupported_content)
    ...
    def _on_load_finished(self, successful):        
        ...
        #collection = self.webframe.findAllElements("img")
        #print "element count: %d" % collection.count()
        #collectionList = collection.toList()
        #for el in collectionList:
        #   print el.attribute("src")
        #   print "%s image width: %d px" % (el.attribute("src"), el.geometry().size().width())
        #   print "%s image height: %d px" % (el.attribute("src"), el.geometry().size().height())
        #      """ """QPixmap pix(el.geometry().size());
        #         QPainter p(&pix);
        #         el.render(&p);
        #         ui->m_lbl->setPixmap(pix);
        #         return;"""
